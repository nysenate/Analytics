...
public class OpenLegislationConnect
{
    ...
    public static String get(String value) throws Exception
    {
        if (value.contains("/transcript/")) {
            ...
            /*
             * Pattern pattern =
             * Pattern.compile("(\\?|&)\\w*?=[\\w\\W&&[^&]]*"); Matcher m =
             * pattern.matcher(value); String ret = "";
             * 
             * while(m.find()) { String term =
             * value.substring(m.start(),m.end()); try { String termValue =
             * term.split("(\\?|&)\\w*?=")[1]; String termStripped =
             * term.replaceAll("(&|\\?|=[\\w\\W]*)", "");
             * 
             * ret += (!ret.equals("") ? "; " : "") + termStripped + ": " +
             * termValue; } catch (Exception e) {
             * 
             * }
             * 
             * }
