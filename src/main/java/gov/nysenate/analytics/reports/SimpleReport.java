...
public class SimpleReport {
	...
	public static boolean generateCSV(GoogleAnalyticsConnect gac, Section params) {
		try {
			...
			// This requires some ugly hacking becaues we've got some non-standard values and formats
	        DataEntry firstEntry = entries.get(0);
	        List<String> feedDataNames = new ArrayList<String>();
	        List<String> dimensionNames = new ArrayList<String>();
	        
	        // header is a special string used to hack around some custom column names from openleg
	        String header = "";
	        
	        for (Dimension dimension : firstEntry.getDimensions()) {
	        	dimensionNames.add(dimension.getName());
	        	
	        	// if this output needs to have an openleg field, hack it in, we'll retrieve the value later
	        ...
	        //If date wasn't a specified dimension hack that in also, we'll provide a dummy value later
