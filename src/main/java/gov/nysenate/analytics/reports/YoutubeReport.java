		...
		try {
			...
			for(NYSenate temp:nySenateData) {
				try {
					...
					BufferedReader br = Utils.getURLWithQueryStringReader(temp.youtubeURL.concat("/videos?sort=p&flow=list&page=1&view=0"));//append sorting option
					...
					while((webLine = br.readLine()) != null) {
						...
						//statistical data found read it to process when description will be fetched in next line
						...
						//all data found for this URL break now to enhance performance and avoiding overwriting of TopVideo name
						...
						//backup data inside buffer till it will be processed after breaking loop
						if(isFeedItemFound){
					...
					//today's Date,Channel/first name,Videos,Lifetime ChannelViews,Subscribers,Top Viewed,Views to Date
					bw.write(
							today + ","
